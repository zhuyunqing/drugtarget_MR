...
###########
#table 1-根据grs的4分位数分组 描述基线特征


m <- fread("/Users/zhuyunqing/Desktop/study/drugMR_lipid/data/onesamplemr/CAPOC.cis.capoc_cases.grs.pheno.new.txt", h=T)

m$medication_metabo <- ifelse((m$medication==1 | m$medication==2 | m$medication==3), 1,2 )
table(m$medication_metabo)
...
#地区 教育
m <- m %>%
  mutate(centers_south = ifelse((centers==2 | centers==3), 1, 0),
         edu_lower_2g = ifelse((edu_4g == "middle" | edu_4g == "primary_or_lower"), 1, 0))



# 定义变量列表
variables <- c("centers_south", "edu_lower_2g", "marry_2g", "drugp07", "first_episode_2g", "medication_metabo") 
...
# 定义连续变量列表
continuous_variables <- c("course_years", 
                          "bmi0601") 
...
# 合并所有结果
final_results_continuous1 <- rbindlist(results_continuous_list, use.names = TRUE, fill = TRUE)

# 查看结果
print(final_results_continuous1)
...
# 定义连续变量列表
continuous_variables <- c("pls0601", "qtc0801","sbp0601", "dbp0601") 
...
# 合并所有结果
final_results_continuous3 <- rbindlist(results_continuous_list, use.names = TRUE, fill = TRUE)

# 查看结果
print(final_results_continuous3)
...
# 连续变量列表
continuous_variables <- c("tc1_mg", "tg1_mg", "hdl1_mg", "ldl1_mg", "glu1_mg", 
                          "alt1","ast1","bun1","cre1","prl1","hba1")
...
#combine为table1
table1_all <- rbind(male_count_ratio, female_count_ratio, age_median_iqr, final_results_cate, final_results_continuous1, final_results_continuous3, final_results_continuous2)
...
#combine为table1
table1_APOC3 <- rbind(male_count_ratio, female_count_ratio, age_median_iqr, final_results_cate, final_results_continuous1, final_results_continuous3, final_results_continuous2)
...
#combine为table1
table1_GCK <- rbind(male_count_ratio, female_count_ratio, age_median_iqr, final_results_cate, final_results_continuous1, final_results_continuous3, final_results_continuous2)
...
model <- lm(m$grs_APOC3_w_glgc_logTG ~ m$sex + m$age + m$centers + m$PC1 + m$PC2 + m$PC3 + m$PC4 + m$PC5)
summary(model)
#仅地区显著

# 定义要替换的变量
variables <- c("centers_south", "edu_lower_2g", "marry_2g","first_episode_2g", "medication_metabo", 
               "drugp07", "age_first_episode", "course_years",
               "bmi0601", "pls0601","sbp0601", "dbp0601", 
               "tc1_mg", "tg1_mg", "hdl1_mg", "ldl1_mg", "glu1_mg"
)
p_values_df <- data.frame(variable = character(), p_value = numeric(), stringsAsFactors = FALSE)

for (var in variables) {
  formula <- as.formula(paste("m$", var, " ~ m$grs_APOC3_w_glgc_logTG+ m$sex + m$age + m$age2 + m$centers + m$PC1 + m$PC2 + m$PC3 + m$PC4 + m$PC5", sep = ""))
...
model <- lm(m$grs_GCK_w_taiwan_glu ~ m$sex + m$age + m$centers + m$PC1 + m$PC2 + m$PC3 + m$PC4 + m$PC5)
summary(model)
#仅地区显著

# 定义要替换的变量
variables <- c("edu_lower_2g", "marry_2g","first_episode_2g", "medication_metabo", 
               "drugp07", "age_first_episode", "course_years",
               "bmi0601", "pls0601","sbp0601", "dbp0601", 
               "tc1_mg", "tg1_mg", "hdl1_mg", "ldl1_mg", "glu1_mg"
)
...
for (var in variables) {
  formula <- as.formula(paste("m$", var, " ~ m$grs_GCK_w_taiwan_glu+ m$sex + m$age + m$age2 + m$centers + m$PC1 + m$PC2 + m$PC3 + m$PC4 + m$PC5", sep = ""))
  model <- lm(formula, data = m)
  p_value <- summary(model)$coefficients[2, "Pr(>|t|)"]
  p_values_df <- rbind(p_values_df, data.frame(variable = var, p_value = p_value))
}
...
fwrite(tables, "/Users/zhuyunqing/Desktop/study/drugMR_lipid/result/table1.grs_4g.des.csv")
fwrite(p_values, "/Users/zhuyunqing/Desktop/study/drugMR_lipid/result/table1.grs_4g.des.p.csv")
